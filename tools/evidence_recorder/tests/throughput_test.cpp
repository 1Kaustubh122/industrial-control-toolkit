...
int main(){
    ...
    ictk::tools::RecorderOptions opt; opt.out_dir="evidence_tp"; opt.segment_max_mb=32; opt.dt_ns_hint=1'000'000;
    ...
    const int hz=1000, secs=5; const long long dt = 1'000'000'000ll/hz;
    long long t = t0m;
    for(int i=0;i<hz*secs;i++){ ictk::tools::TickSample s{}; s.t=t; rec->write_tick(s); t+=dt; rec->rotate_if_needed(); }
    rec->write_kpi({}); rec->flush(); return 0;
#endif
}

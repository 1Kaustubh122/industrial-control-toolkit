...
int main(){
    ...
    */
    ...
    [[maybe_unused]] const double u1 = u[0];
    ps.t += dt; y[0] = 0.3; 
    assert(pid.update({ps,sp},res)==Status::kOK); 
    [[maybe_unused]] const double u2= u[0];

    [[maybe_unused]] const double dKp = (3.0-1.0)*(0.1); 
    [[maybe_unused]] const double bound = std::abs(dKp * (1.0 - 0.3));
